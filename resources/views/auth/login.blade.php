...
@php
$inputs = [
    ['type' => 'email', 'name' => 'email', 'label' => 'Dirección de correo'],
...
<div class="px-2 sm:px-0">
    <div class="border bg-white dark:bg-gray-800 border-gray mx-auto w-full sm:w-400 px-8 py-4 rounded-sm my-20">
        <form method="post">
            ...
            <h1 class="my-2 font-bold text-xl text-current-600 dark:text-current-400">Acceso aplicación {{ config('app.name') }}</h1>
                ...
                <div class="my-2 mb-4 flex items-center justify-between" style="margin-top: -4px">
                    <div>
                        <label class="block text-gray-400" for="remember">
                            ...
                            <span class="text-gray-600 dark:text-gray-300 font-bold text-xs select-none cursor-pointer">Recuerdame</span>
                        </label>
                    </div>

                    <div>
                        <a href="{{ route('password.request') }}" class="transition text-xs text-current-600 hover:text-current-500 dark:text-current-400 dakr:hover:text-current-500 font-bold">Contraseña olvidada</a>
                    </div>
                </div>
        ...
        <div class="mt-4 mb-2 flex justify-center">
            <p class="font-bold text-xs text-gray-600 dark:text-gray-300">No tienes una cuenta?</p>&nbsp;
            <a href="{{ route('join') }}" class="transition font-bold text-xs text-current-600 hover:text-current-500 dark:text-current-400 dark:hover:text-current-500">Crear</a>
        </div>
    </div>
</div>
@stop